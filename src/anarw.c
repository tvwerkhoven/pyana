...
int ck_synch_hd(int fd,fzhead_t **fho,int t_endian)
{
  int wwflag=0;
  struct fzhead fh;
  if(read(fd,&fh,sizeof(fzhead_t))!=sizeof(fzhead_t)){
...
//
  *fho=malloc(fh.nhb*sizeof(struct fzhead));
  (*fho)[0]=fh;
//
  if(syncpat==t_endian){
  ...
  if(fh.nhb>1){                     // if the header is long, read in the rest now
    if(fh.nhb>15){
      ...
      free(*fho); // not very useful?
      *fho=0;
      return -1;
    }
    int size=(fh.nhb-1)*sizeof(fzhead_t);
    if(read(fd,(*fho)+1,size)<size){
      free(*fho); // not very useful?
      *fho=0;
      fprintf(stderr,"ck_synch_hd: unexpected end of file!\n");
      return -1;
  }
  }
  if(t_endian) bswapi32((*fho)->dim,(*fho)->ndim); // for big endian machines
  return wwflag; 
}
...
//
  ...
  if(posix_fadvise(fd,0,stat_buf.st_size,POSIX_FADV_NOREUSE|POSIX_FADV_SEQUENTIAL)<0) fprintf(stderr,"ana_fzhead: %s\n",strerror(errno));
//
  struct fzhead	*fh;
  int sef;
  if((sef=ck_synch_hd(fd,&fh,t_endian))<0){
    if(posix_fadvise(fd,0,stat_buf.st_size,POSIX_FADV_DONTNEED)<0) fprintf(stderr,"ana_fzhead: %s\n",strerror(errno));
    close(fd);
    return 0;
  }
/**/
  ...
  header[0]=0;
  for(i=0;i<fh->nhb;++i){
    char *tmp=malloc(strlen(header)+strlen(fh[i].txt)+1);
...
/**/
  ...
  *type=fh->datyp;
  ...
  if(compressed){                                  // compressed format
    ...
    if(read(fd,&ch,14)<14) fprintf(stderr,"error reading in compression header\n");
...
// read data
    ...
    if(read(fd,buf,size)<size) fprintf(stderr,"error reading in compressed data\n");
//
    if(posix_fadvise(fd,0,stat_buf.st_size,POSIX_FADV_NOREUSE|POSIX_FADV_SEQUENTIAL)<0) fprintf(stderr,"ana_fzhead: %s\n",strerror(errno));
    close(fd);
//
...
//
  ...
  struct fzhead *fh=malloc(nhead*sizeof(struct fzhead));
...
//      case(INT64): fprintf(stderr,"ana_fzwrite: error: ana_fzwrite: unknown variable type!\n");
  ...
  int i;
  for(i=0;i<=3;++i) fh->cbytes[i]=((unsigned char*)&res)[i];
...
/* extended to 32 bits 2/4/96 */
  ...
  int i;
  for(i=0;i<=3;++i) fh->cbytes[i]=((unsigned char*)&res)[i];
  fwrite(fh,1,nhead*sizeof(struct fzhead),f);  // write header
  fwrite(q,1,size,f);
  free(q);
  free(fh);
  fclose(f);
